...
impl Iso8583TransactionService {
    ...
    pub async fn process_transaction(
        ...
        let mut request_msg = self.build_iso_message(card_request, &stan)?;

        // 3. Save transaction to database
        let mut db_transaction = self.create_db_transaction(&request_msg, card_request)?;
        ...
        // 5. Parse response
        let (state, response_code) = ResponseHandler::parse_response(&response_msg);
